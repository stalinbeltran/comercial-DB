...
```powershell
mysql -u root -p < db/create/01_create_schema.sql
```

Crea la base de datos `comercial_db` con todas las tablas, índices y FK.
...
$env:SEED_SCALE=1000; python scripts/seed_massive.py

# Semilla aleatoria distinta en cada ejecución
$env:SEED_RANDOM=0; python scripts/seed_massive.py

# Limpiar primero y luego poblar en un solo flujo
python scripts/clear_db.py --yes; if ($?) { python scripts/seed_massive.py }
```
...
Usa `comercial_db` (sin sufijo `_test`). Inyecta 10 escenarios de prueba de negocio (ESC-01..ESC-10).
...
```powershell
# 1. Crear esquema en DB de test
mysql -u root -p comercial_db_test < db/create/01_create_schema.sql
...
### Recarga de datos masivos y tests de regresión

```powershell
python scripts/clear_db.py --yes; if ($?) { python scripts/seed_massive.py }; if ($?) { pytest }
```

### Inspeccionar DB de desarrollo (datos masivos)

```powershell
# Limpiar comercial_db y repoblar con escala 1000
